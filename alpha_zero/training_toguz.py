...
"""Trains the AlphaZero agent on a single machine for Togyz Kumalak."""
import os
...
flags.DEFINE_integer('min_games', 1000, 'Collect number of self-play games before learning starts.')
flags.DEFINE_integer(
    'games_per_ckpt',
    1000,
...
flags.DEFINE_bool(
    'argument_data',
    False,
    'Apply random rotation and mirroring to the training data. Disabled for Togyz Kumalak since 2D augmentations are not valid.',
)
...
flags.DEFINE_float(
    ...
    -1,
    'Not applicable, as there is no resign move for Togyz Kumalak.',
)
...
flags.DEFINE_string('ckpt_dir', './checkpoints/toguz', 'Path for checkpoint file.')
flags.DEFINE_string(
    'logs_dir',
    './logs/toguz',
...
flags.DEFINE_integer(
    ...
    0,
    'The frequency (in number of self-play games) to save the replay buffer state. 0 means do not save.',
)
...
def main():
    ...
    def env_builder():
        return ToguzKumalakEnv(num_stack=FLAGS.num_stack)
